...
client = Client("no_verif_bot", api_id, api_hash, bot_token=bot_token)
mongo = MongoClient(mongo_url)
db = mongo["verifybot"]
users = db["verified_users"]  # You can remove this too if not used elsewhere
...
@client.on_message(
    filters.private & ~filters.command(["start", "users", "broadcast", "up"]))
...
async def handle_users(client, message):
    ...
    await message.reply(f"👥 Total Users in DB (not used): `{total}`")
...
print("Bot running without verification...")
client.run()
